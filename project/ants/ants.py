...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        if isinstance(insect,QueenAnt) and insect.iftrue:
            return
        if insect.is_ant:
...
class Ant(Insect):
    ...
    def can_contain(self,otherant):
        return self.container and self.ant==None and not otherant.container
    ...
    def nearest_bee(self, hive):
        ...
        while current_place != hive:
            if current_place.bees:
                if transitions in range(self.min_range, self.max_range+1):
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting:
            ...
            _bees = self.place.bees
            self.eat_bee(random_or_none(_bees))
        # END Problem 10
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        Ant.__init__(self, 1)
        self.lst=[]
        if not QueenAnt.ifhavetrue:
            self.iftrue=True
            QueenAnt.ifhavetrue=True
        else:
            self.iftrue=False
        # END Problem 13
    ...
    def action(self, colony):
        ...
        _place=self.place.exit
        while _place!=None:
            if _place.ant!=None:
                if not _place.ant in self.lst:
                    self.lst.append(_place.ant)
                    _place.ant.damage+=_place.ant.damage
                if _place.ant.container and _place.ant.ant!=None:
                    if not _place.ant.ant in self.lst:
                        self.lst.append(_place.ant.ant)
                        _place.ant.ant.damage+=_place.ant.ant.damage
            _place=_place.exit
        # END Problem 13
