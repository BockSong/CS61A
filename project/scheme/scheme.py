    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) < len(vals):
            ...
            if formals is not nil:
                child = self.make_child_frame(formals.second, vals.second)
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Arguments error')
        # END PROBLEM 4
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        resulting expanded expressionn is to be evaluated."""
        # BEGIN Problem 22
        expr=scheme_apply(self,operands,env)
        return scheme_eval(complete_eval(expr),env)
        # END Problem 22
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        if not scheme_symbolp(target.first):
            raise SchemeError
        lambda_expr = Pair(target.second, expressions.second)
...
def scheme_call_cc(function, env):
    ...
    # BEGIN PROBLEM 21
    try:
        args = Pair(ContinuationProcedure(ContinuationError), nil)
        frame = function.env.make_child_frame(function.formals, args)
        sub_result = eval_all(function.body, frame)
        scheme_eval(sub_result.expr, sub_result.env)
