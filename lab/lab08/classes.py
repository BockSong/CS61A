...
class Player(object):
    ...
    def talk_to(self, person):
        ...
        >>> kevin = Character('Instructor Kevin', 'Have to run for lecture!')
        >>> sather_gate = Place('Sather Gate', 'You are at Sather Gate', [kevin], [])
        >>> me = Player('player', sather_gate)
        >>> me.talk_to(kevin)
        Person has to be a string.
        >>> me.talk_to('Instructor Kevin')
        Instructor Kevin says: Have to run for lecture!
        >>> me.talk_to('Albert')
        Albert is not here.
        """
        if type(person) != str:
